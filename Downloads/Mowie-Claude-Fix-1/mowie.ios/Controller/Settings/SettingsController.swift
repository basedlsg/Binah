...
//  mowie.ios
//
//  Created by Treyvon Cortez Johnson on 12/26/23.
//
...
class SettingsController: UITableViewController {
    ...
    private lazy var infoHeader: UserInfoHeader = {
        let frame = CGRect(x: 0, y: 0, width: view.frame.width - 80, height: 100)
    ...
    func configureNavigationBar() {
        ...
        navigationItem.leftBarButtonItem = UIBarButtonItem(image: UIImage(systemName: "gearshape"), style: .plain, target: self, action: #selector(handleDismissal))
    }
}
...
extension SettingsController: AddLocationControllerDelegate {
    func updateLocation(locationString: String, type: LocationType) {
        Service.shared.saveLocation(locationString: locationString, type: type) { (err, ref) in self.dismiss(animated: true, completion: nil)
