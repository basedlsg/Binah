...
import { ItineraryPlace, ItineraryVenue, PlanResponse, PlannedActivity } from "./types.js";

const CONFIG = {
  GOOGLE_PLACES_API_KEY:
    process.env.GOOGLE_PLACES_API_KEY || (functions.config()?.planner?.google_places_api_key as string | undefined) || "",
  GEMINI_API_KEY:
    process.env.GEMINI_API_KEY || (functions.config()?.planner?.gemini_api_key as string | undefined) || "",
  CORS_ORIGIN:
    process.env.CORS_ORIGIN || (functions.config()?.planner?.cors_origin as string | undefined) || ""
};
...
app.get("/api/cities", (_req, res) => {
  res.status(200).json(
    cities.map((c) => ({ slug: c.slug, name: c.name, timezone: c.timezone }))
  );
});
...
app.post("/api/plan", planLimiter, async (req, res) => {
  ...
  try {
    const activities: PlannedActivity[] = await gemini.planActivities({
      query: String(query),
      city,
      date: String(date),
...
function buildScheduleTimes(count: number, startHHmm: string): string[] {
  const result: string[] = [];
...
function buildTitle(query: string, cityName: string, date: string): string {
...
function buildGenericVenues(query: string, city: { defaultArea: string; name: string }, times: string[], count: number): ItineraryVenue[] {
  const items: ItineraryVenue[] = [];
