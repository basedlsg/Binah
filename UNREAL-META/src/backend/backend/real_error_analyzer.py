...
class RealErrorAnalyzer:
    ...
    def _load_error_patterns(self) -> Dict[str, List[Dict[str, Any]]]:
        ...
        return {
            "plugin_conflict": [
                {
                    "pattern": r"Multiple XR plugins detected",
                    "indicators": ["OpenXR", "MetaXR", "OculusXR", "plugin conflict"],
                    "severity": "critical",
                    "confidence": 0.95
                },
                {
            ...
            "black_screen": [
                {
                    ...
                    "indicators": ["Failed to allocate", "VR eye buffer", "Vulkan"],
                    "severity": "critical",
                    "confidence": 0.91
                },
                {
            ...
            "memory_leak": [
                {
                    "pattern": r"Memory leak detected in VR subsystem",
                    "indicators": ["memory leak", "VR subsystem", "Quest"],
                    "severity": "high",
                    "confidence": 0.85
                },
                {
                    "pattern": r"Out of memory.*Quest",
                    "indicators": ["out of memory", "Quest", "allocation"],
                    "severity": "critical",
                    "confidence": 0.90
                }
            ],
            "shader_compile": [
                {
                    ...
                    "indicators": ["shader", "compilation", "failed", "HLSL"],
                    "severity": "medium",
                    "confidence": 0.88
                },
                {
                    "pattern": r"HLSL error.*undeclared identifier",
                    "indicators": ["HLSL", "error", "undeclared", "identifier"],
                    "severity": "medium",
                    "confidence": 0.85
                }
            ]
        ...
        return {
            ...
            "packaging_error": [
                ErrorSolution(
                    ...
                    title="Fix Build Failed AutomationTool Errors",
                    description="BUILD FAILED errors with AutomationTool are typically caused by packaging configuration issues, missing dependencies, or build environment problems.",
                    steps=[
                        "Clean your project build cache",
                        "Check Project Settings → Packaging → Advanced",
                        "Verify Android SDK and NDK paths are correct",
                        "Ensure all required plugins are enabled",
                        "Check for missing content references",
                        "Try packaging with Development configuration first",
                        "Review AutomationTool log for specific error details"
                    ],
                    config_changes={
                        "DefaultEngine.ini": {
                    ...
                    verification_steps=[
                        "Check that AutomationTool completes without errors",
            ...
            "shader_compile": [
                ErrorSolution(
                    ...
                    description="Shader compilation failures are often caused by HLSL syntax errors, missing includes, or incompatible shader features.",
                    steps=[
                        "Check the specific HLSL error message",
                        "Verify all shader includes are present",
                        "Check for typos in variable/function names",
                        "Ensure shader features are supported on target platform",
                        "Review shader code for Quest-specific limitations",
                        "Try compiling shaders individually to isolate issues"
                    ],
                    verification_steps=[
                        "Check that shader compilation completes without errors",
                        "Verify materials render correctly in editor",
                        "Test shader performance on Quest device"
                    ],
                    confidence=0.85
                )
            ]
    ...
    def _detect_multiple_errors(self, log_content: str) -> List[Dict[str, Any]]:
        ...
        for error_type, patterns in self.error_patterns.items():
            ...
            if best_match and max_confidence > 0.7:  # Threshold for inclusion
    ...
    def _analyze_unknown_error(self, log_content: str) -> Dict[str, Any]:
        ...
        error_keywords = {
            "packaging": ["packaging", "cook failed", "build failed"],
            "rendering": ["render", "shader", "material", "vulkan"],
            "memory": ["memory", "allocation", "out of memory"],
            "android": ["android", "apk", "gradle"],
            "vr": ["vr", "xr", "headset", "quest"]
        }
        ...
        if category_scores:
            ...
            confidence = min(0.6, category_scores[likely_category] * 0.1)
        else:
        ...
        return {
            ...
            "recommendation": "Manual analysis required - error pattern not recognized"
        }
